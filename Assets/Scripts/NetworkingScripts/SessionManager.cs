...
public class SessionManager<T> where T : ISessionPlayerData
{
    SessionManager()
    {
    ...
    public void DisconnectClient(ulong clientId)
    {
        ...
        if (m_HasSessionStarted)
        {
            ...
            if (!m_ClientIDToPlayerId.TryGetValue(clientId, out var playerId) ||
                GetPlayerData(playerId)?.ClientID != clientId) return;
        ...
        else
        {
            ...
            if (GetPlayerData(playerId)?.ClientID != clientId) return;
    ...
    /// <summary>
    /// Adds a connecting player's session data if it is
    /// a new connection, or updates their session data in case of a reconnection.
    /// </summary>
    /// <param name="clientId">This is the clientId that Netcode assigned us on login.
    /// It does not persist across multiple logins from the same client. </param>
    /// <param name="playerId">This is the playerId that is unique to this client
    /// and persists across multiple logins from the same client</param>
    ...
    {
        Debug.Log($"SetupConnectingPlayerSessionData {clientId}, {playerId}");
        ...
        if (m_ClientData.TryGetValue(playerId, out var value) && !value.IsConnected)
        {
        ...
        Debug.Log("Completed SetupConnectingPlayerSessionData");
    }
    ...
    public void SetPlayerData(ulong clientId, T sessionPlayerData)
    {
        ...
        Debug.Log($"Setting data for {clientId}");
    }
